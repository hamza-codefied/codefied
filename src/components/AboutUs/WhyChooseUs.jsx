          ...
          <div className='flex flex-col md:flex-row gap-[40px] md:gap-[74px]'>
            ...
            <div className='space-y-4 w-full md:w-[530px]'>
              {items.map((item, index) => (
                <div
                  key={index}
                  className={`relative flex justify-between items-center border-b pb-2 cursor-pointer transition-all duration-300 ${
                    activeIndex === index
                      ? 'border-[#D4575B]'
                ...
                >
                  ...
                  <ArrowRight
                    className={`w-[20px] h-[20px] md:w-[25px] md:h-[25px] transition-transform ${
                      activeIndex === index
                        ? 'translate-x-1 text-[#D4575B]'
                        : ''
                    }`}
                  />
                  {activeIndex !== index && (
                    <span
                      className='absolute bottom-0 left-0 w-0 h-[2px] bg-[#D4575B] transition-all duration-300 group-hover:w-full z-10'
