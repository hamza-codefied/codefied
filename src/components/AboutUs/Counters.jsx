    ...
    >
      <div className='max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-[14px] md:gap-8 text-center relative z-10'>
        {counters.map((item, index) => {
          ...
          const counterInView = useInView(counterRef, { once: true, amount: 0.1 });
          ...
          return (
            <motion.div
              ...
              animate={counterInView ? { opacity: 1, y: 0 } : { opacity: 1, y: 0 }}
              transition={{ duration: 0.6, delay: index * 0.2 }}
              className={`bg-transparent p-6 ${index === 0 || index === 2
            ...
            >
              ...
              <p className='text-gray-700 font-medium' style={{ fontSize: 'clamp(12px, 2vw, 16px)' }}>{item.label}</p>
            </motion.div>
          );
        })}
      </div>
